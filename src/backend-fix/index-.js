client@ik-019a55e2-8e41-70b7-b7b5-2734289e1bfd-6987778968-njnw7:~/sites/api.thomasmaury.fr$ cat /tmp/index-current.js
...
app.post('/chat-text', async (req, res) => {
  try {
    const { message, conversation_history = [], isPremium = false } = req.body;
    ...
    if (model === 'claude') {
      const response = await axios.post(
        'https://api.anthropic.com/v1/messages',
        {
          model: 'claude-sonnet-4-20250514',
          max_tokens: 1000,
          messages: conversation_history.concat([{ role: 'user', content: message }])
        },
...
client@ik-019a55e2-8e41-70b7-b7b5-2734289e1bfd-6987778968-njnw7:~/sites/api.thomasmaury.fr$ ^C
client@ik-019a55e2-8e41-70b7-b7b5-2734289e1bfd-6987778968-njnw7:~/sites/api.thomasmaury.fr$ 